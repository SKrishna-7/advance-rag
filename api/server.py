...
async def chat_stream_endpoint(request: QueryRequest):
    ...
    async def event_generator():
        try:
            ...
            # ==========================================
            if intent == "CHAT":
                # 1. Get the full response from your new DSPy engine
                full_response = rag_bot.generator.chat_casually(request.question)
            
                # 2. Simulate the token stream for your React frontend
                ...
                for i, word in enumerate(words):
                    ...
                    # Tiny yield to simulate typing speed and prevent async blocking
                    await asyncio.sleep(0.02) 
                
                # THESE MUST BE OUTSIDE THE FOR LOOP!
                yield "data: [DONE]\n\n"
                return
            ...
            if len(docs) > 5: docs = docs[:5]
            
            
            # 3. Format Sources (But do NOT yield them yet!)
            formatted_sources = []
            for doc in docs:
                img_data = doc.metadata.get("images", [])
                final_imgs = []
            ...
            # 4. Generate & Stream Answer First!
            full_answer = ""
            for token in rag_bot.generator.native_stream_answer(request.question, docs):
                full_answer += token
                token_payload = json.dumps({"type": "token", "content": token})
                yield f"data: {token_payload}\n\n"

                await asyncio.sleep(0)
            # 5. Yield Sources & Images Now that Text is Done
            ...
            logger.error(f"Streaming Error: {e}")  # Uncomment if you have your logger imported
